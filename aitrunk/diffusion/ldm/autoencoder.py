...
COMPVIS_AUTOENCODER_MODEL_KEYS = ['kl-f4', 'kl-f8', 'kl-f16', 'kl-f32']
COMPVIS_AUTOENCODER_BASEDIR = '/home/hym/trunk/latent-diffusion/models/first_stage_models'


def load_compvis_autoencoder(model_key='kl-f4', device='cpu'):
    ckpt_path = f'{COMPVIS_AUTOENCODER_BASEDIR}/{model_key}/model.ckpt'
    config_path = f'{COMPVIS_AUTOENCODER_BASEDIR}/{model_key}/config.yaml'
    with open(config_path, 'rb') as f:
        config = yaml.safe_load(f)
    params = config['model']['params']
    encoder = Encoder(z_channels=params['ddconfig']['z_channels'],
                      in_channels=params['ddconfig']['in_channels'],
                      channels=params['ddconfig']['ch'],
                      channel_multipliers=params['ddconfig']['ch_mult'],
                      n_resnet_blocks=params['ddconfig']['num_res_blocks'])
    decoder = Decoder(out_channels=params['ddconfig']['out_ch'],
                      z_channels=params['ddconfig']['z_channels'],
                      channels=params['ddconfig']['ch'],
                      channel_multipliers=params['ddconfig']['ch_mult'],
                      n_resnet_blocks=params['ddconfig']['num_res_blocks'])
    autoencoder = Autoencoder(emb_channels=params['embed_dim'],
                              encoder=encoder,
                              decoder=decoder,
                              z_channels=params['ddconfig']['z_channels'])
...
class AETest(unittest.TestCase):
    def test_load_compvis_autoencoders(self):
        for key in COMPVIS_AUTOENCODER_MODEL_KEYS:
            ae, config = load_compvis_autoencoder(key)
