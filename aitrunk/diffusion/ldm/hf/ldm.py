...
class LDM(nn.Module):
    def __init__(self, config=None):
        ...
        self.beta = torch.linspace(beta_start ** 0.5, beta_end ** 0.5, self.n_steps) ** 2
        alpha = 1. - self.beta
    ...
    def loss_and_log(self, batch, log_prefix='train'):
        loss = self.compute_loss(x0=batch['input'], c=batch['cond'], uc=batch.get('uncond'))
        self.log(f'{log_prefix}.loss', loss, prog_bar=True)
        return loss

    def compute_loss(self, x0, noise=None, c=None, uc=None):
        """
