...
from aitrunk.diffusion.ldm.sampler import DDPMSampler
...
class DiffusionLitModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        return self._loss_and_log(batch, batch_idx, log_prefix='train')

    def validation_step(self, batch, batch_idx):
        return self._loss_and_log(batch, batch_idx, log_prefix='val')
    ...
    def create_sampler(self, method='ddpm'):
        if method == 'ddpm':
            return DDPMSampler(model=self)
        else:
            raise NotImplementedError(f'Unknown sampling method: {method}')

    def _loss_and_log(self, batch, batch_idx, log_prefix='train'):
        loss = self._get_loss(x0=batch['input'],
                              c=batch['cond'],
                              uc=batch.get('uncond'),
                              uc_scale=batch.get('uncond_scale', 1.))
        self.log(f'{log_prefix}.loss', loss, prog_bar=True)
        return loss

    def _get_loss(self, x0, noise=None, c=None, uc=None, uc_scale=1.):
        """
        ...
        :param uc_scale: is the scale for unconditional guidance
        """
        ...
        eps_theta = self.get_eps(xt, t, c, uc, uc_scale)

        # MSE loss
        return F.mse_loss(noise, eps_theta)

    def get_eps(self, x, t, c, uc, uc_scale=1.):
        """
        ...
        return eps_uc + uc_scale * (eps_c - eps_uc)
