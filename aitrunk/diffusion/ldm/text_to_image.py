...
class Txt2Img:
    ...
    def __call__(self, *,
        ...
        with torch.cuda.amp.autocast():
            ...
            # cond = self.model.get_text_conditioning(batch_size * [""])

            # [Sample in the latent space](../sampler/index.html).
            # `x` will be of shape `[batch_size, c, h / f, w / f]`
            x = self.sampler.sample(x_shape=(c, h // f, w // f),
                                    cond=cond,
...
def main():
    ...
    ckpt_path = '/data/ldm/sd-v1-4.ckpt'
    txt2img = Txt2Img(checkpoint_path=ckpt_path,
                      sampler_name=opt.sampler_name,
                      n_steps=opt.steps)

    with monit.section('Generate'):
        txt2img(dest_path='output',
